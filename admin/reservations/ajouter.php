...
try {
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        // Vérifier que la chambre n'est pas réservée
        $sql_check = "SELECT COUNT(*) as count FROM reserver WHERE ID_Chambre = ? 
                      AND ((Date_Arrive_Reserver <= ? AND Date_Dapart_Reserver > ?) 
                           OR (Date_Arrive_Reserver < ? AND Date_Dapart_Reserver >= ?))";
        $stmt_check = $conn->prepare($sql_check);
        $stmt_check->execute([$id_chambre, $date_depart, $date_arrive, $date_depart, $date_arrive]);
        ...
        $stmt_insert = $conn->prepare("
            INSERT INTO reserver (ID_Client, ID_Chambre, Date_Arrive_Reserver, Date_Dapart_Reserver, 
                                  Etat_Reserver, Prix_Total, Date_Reservation_Reserver)
            VALUES (?, ?, ?, ?, ?, ?, NOW())
        ");
        $stmt_insert->execute([$id_client, $id_chambre, $date_arrive, $date_depart, $etat, $prix_total]);

        // Marquer la chambre comme occupée
        $conn->prepare("UPDATE chambre SET Disponible = 0 WHERE ID_Chambre = ?")->execute([$id_chambre]);

        $message = "✅ Réservation créée avec succès";
        $message_type = "success";
        
        // Redirection après 2 secondes
        header("refresh:2;url=index.php?added=1");
    }
    ...
    <form method="POST">
            ...
            <select name="id_chambre" id="id_chambre" required onchange="updatePrice()">
                ...
                <?php foreach ($chambres as $chambre): ?>
                    <option value="<?= (int)$chambre['ID_Chambre'] ?>" 
                            ...
                            data-disponible="<?= (int)$chambre['Disponible'] ?>">
        ...
        <div class="form-row">
            <div class="form-group">
                ...
                <input type="date" name="date_arrive" id="date_arrive" onchange="updatePrice()" required>
            </div>

            <div class="form-group">
                <label for="date_depart">📅 Date de départ:</label>
                <input type="date" name="date_depart" id="date_depart" onchange="updatePrice()" required>
            </div>
        </div>
